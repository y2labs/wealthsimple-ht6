...
export const getNextPetAttributes = pet => {
  ...
  const nextHunger = max([
    round(
      random(
        pet.hunger - 5 * Math.min(CONSTANTS.maxAttributeValue / pet.hunger, 2),
        pet.hunger
      )
    ),
    0
  ...
  const nextEnergy = max([
    round(random(pet.energy - 5 * multipliers.hunger, pet.energy)),
    0
  ]);
  ...
  const nextContent = round(
    max([random(pet.content - 5 * multipliers.energy, pet.content), 0])
  );
